 ...
 */
public class DeployEshowTask extends TaskBase {
    ...
    public void run() {
        ...
        if(working){
            this.sendChat("eshow:duplicate command!");
            return;
        }
        try {
            working=true;
    ...
    private void checkShellFile(String workingDir,String shellName){
        ...
        if(!shellFile.exists()|| force!=null) {
            try {
                ...
                FileUtils.copyInputStreamToFile(DeployEshowTask.class.getClassLoader().getResourceAsStream( shellName), shellFile);
